...
| Phase 2.2G.3 | Learning Ecosystem — scholarly learning graph (programmes, curricula, courses, modules, lessons, topics, activities, assessments, competency framework, certificates, badges, portfolios, mentorships, CPD, reading lists) with ten pure engines (core, adaptive, analytics, contracts, notifications, permissions, portfolio, recommendations, validation, workflows), canonical placeholder data referencing canonical researchers by username, a single `useLearning` hook, learning components (learner, educator, mentor, programme manager, administration dashboards), and 17 `/learning` routes. | Complete | `TBD` | `TBD` | Passed | None required | Module: `types/learning.ts`, `lib/learning.ts` + `lib/learning-*.ts` (10 engines) + `lib/index.ts`, `constants/placeholder-learning.ts`, `constants/placeholder-researchers.ts`, `hooks/useLearning.ts`, `components/learning/` (incl. `admin/`, `workspace/`), routes `/learning` and all `/learning/*` (17 pages). |
