...
import type { ConferenceFitRecommendation } from '@/types/trust';

type ConferenceRecommendationCardProps = {
  recommendation: ConferenceFitRecommendation;
};
  ...
  return (
    <article className="flex flex-col rounded-[1.75rem] border border-slate-200 bg-white p-6 shadow-[0_20px_60px_-40px_rgba(15,23,42,0.15)]">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <h3 className="text-lg font-semibold leading-7 text-slate-900">{recommendation.conferenceName}</h3>
          <p className="mt-1 text-xs font-medium text-slate-400">
            {recommendation.country} · {recommendation.dates}
          </p>
        </div>
        <ScorePill score={recommendation.qualityIndex} />
      </div>
      ...
      <div className="mt-4 flex items-center justify-between gap-3 border-t border-slate-100 pt-3 text-sm">
        <span className="text-slate-500">{formatPercent(recommendation.acceptanceRate)} acceptance</span>
