...
import type { ConferenceRelationshipRef, ConferenceRelationships } from '@/types/conference';

type ConferenceRelationshipCardProps = {
  relationships: ConferenceRelationships;
  className?: string;
};

const groups: Array<{ key: keyof ConferenceRelationships; label: string }> = [
...
export default function ConferenceRelationshipCard({ relationships, className = '' }: ConferenceRelationshipCardProps) {
  return (
    <div className={['grid gap-6 md:grid-cols-2 xl:grid-cols-4', className].filter(Boolean).join(' ')}>
      {groups.map((group) => {
        const entries: ConferenceRelationshipRef[] = relationships[group.key];
        return (
