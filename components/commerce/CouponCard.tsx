...
import { CouponStatusBadge } from './CommerceBadge';
import { formatCurrency, formatDate } from './format';
import { validateCoupon } from '@/lib/commerce';
import type { CommerceCoupon } from '@/types/commerce';
...
export default function CouponCard({ coupon, cartTotal = 100 }: CouponCardProps) {
  const valueLabel =
    coupon.type === 'percent' ? `${Math.round(coupon.value)}%` : formatCurrency(coupon.value, 'USD');
    ...
    <article className="flex flex-col rounded-[1.75rem] border border-slate-200 bg-white p-6 shadow-[0_20px_60px_-40px_rgba(15,23,42,0.15)]">
      ...
      <div className="mt-3 space-y-1 text-xs text-slate-500">
        {coupon.minimumSpend != null ? (
          <p>Minimum spend {formatCurrency(coupon.minimumSpend, 'USD')}</p>
        ) : null}
        {check.valid ? (
          <p className="font-medium text-emerald-700">
            On a {formatCurrency(cartTotal, 'USD')} cart you save {formatCurrency(check.discount, 'USD')}
          </p>
        ) : (
