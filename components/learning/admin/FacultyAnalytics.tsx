...
export function FacultyAnalytics() {
  ...
  if (rows.length === 0) {
    return <AdminEmptyState title="No faculty analytics" description="Faculty statistics will appear here." />;
  }

  return (
    <>
      <AdminWidgets>
        <StatisticCard icon="🏛️" title="Faculties" value={String(rows.length)} />
        <StatisticCard
          icon="🎓"
          title="Learners"
          value={String(rows.reduce((sum, row) => sum + row.learners, 0))}
          trend="Across tracked faculties"
          trendPositive
        />
      </AdminWidgets>

      <Panel eyebrow="Faculty analytics" title="Aggregated statistics" icon="📈">
