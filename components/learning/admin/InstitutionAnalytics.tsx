...
export function InstitutionAnalytics() {
  ...
  if (rows.length === 0) {
    return <AdminEmptyState title="No institution analytics" description="Institutional statistics will appear here." />;
  }

  return (
    <>
      <AdminWidgets>
        <StatisticCard icon="🏛️" title="Institutions" value={String(rows.length)} />
        <StatisticCard
          icon="🎓"
          title="Learners"
          value={String(rows.reduce((sum, row) => sum + row.learners, 0))}
          trend="Across tracked institutions"
          trendPositive
        />
      </AdminWidgets>

      <Panel eyebrow="Institution analytics" title="Aggregated statistics" icon="📈">
